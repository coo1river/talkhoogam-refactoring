...
export default function ChatDetail() {
  ...
  const findName =
    chatData[chatData.findIndex((item) => item.accountname === params.name)] ||
    "";
    ...
    <LayoutStyle>
        ...
        {findName.messages.map((item, index) => {
          return !!item.receive ? (
            <ChatStyle key={index}>
              ...
              <span className="time">{`${item.createdAt.split(" ")[3]} ${
      ...
      <CommentForm>
        ...
        <CommentInputStyle
          ...
          value={comment} // 입력된 값
        />
        <button className="commnt-btn" disabled={comment.length === 0}>
          전송
        </button>
      </CommentForm>
    </LayoutStyle>
